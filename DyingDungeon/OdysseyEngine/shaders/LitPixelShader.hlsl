...
float calculateShadowFactor(float3 lightDirection, float3 surfaceNormal, float4 shadowPos)
{
    if (shadowsEnabled.x != 1.0f && shadowsEnabled.y != 1.0f)
    {
        return 1.0f;
    }
    ...
    if ((saturate(shadowPos.x) == shadowPos.x) && (saturate(shadowPos.y) == shadowPos.y) && (depth > 0))
    {
		...
		const float staticDX = shadowsEnabled.z;
		const float dynDX = shadowsEnabled.w;
        ...
        for (float x = -1.5f; x < 1.5f; x += 1.0f)
        {
            for (float y = -1.5f; y < 1.5f; y += 1.0f)
            {
				...
				float dynamicDepth = -1.0f;
				float staticDepth = -1.0f;
				float shadowAdd = 0.0f;
				float2 offset = float2(x, y);

				staticDepth = txStaticShadow.Sample(samLinear, shadowPos.xy + offset * shadowsEnabled.z).r;
				dynamicDepth = txDynamicShadow.Sample(samLinear, shadowPos.xy + offset * shadowsEnabled.w).r;
				dynamicDepth = (dynamicDepth > 0.0f) ? dynamicDepth : staticDepth;
				if (depth < dynamicDepth && depth < staticDepth)
					percentLit += 1.0f;
            }
        }
